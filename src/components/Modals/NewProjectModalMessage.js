...
const NewProjectModalMessage = ({ setIsProjectMessageModalOpen }) => {
  return (
    <div className="w-full ml-auto fixed min-h-screen top-0 bg-black bg-opacity-75 z-50 text-center">
      <div className="absolute bg-white rounded-lg shadow top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-11/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        ...
        >
          <svg
            onClick={() => setIsProjectMessageModalOpen(false)}
            className="w-5 h-5"
            fill="currentColor"
            viewBox="0 0 20 20"
        ...
        <div className="py-6 px-6 lg:px-8">
          ...
          <p>The project has been saved successfully.</p>
          <img className="mx-auto w-12 my-6" src={successIcon} alt="success" />
        </div>
      </div>
    </div>
  );
...
export default NewProjectModalMessage;
